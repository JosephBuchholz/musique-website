...
void HarmonyXMLParser::ParseHarmonyXML(const std::string& data, const std::shared_ptr<Song>& song)
{
    LOGI("Parsing HarmonyXML!!!!");
    ...
    if (xmlError != tinyxml2::XMLError::XML_SUCCESS) {
        LOGE_TAG("HarmonyXMLParser", "error: %i", xmlError);
        AddError("XML Error", ToString(int(xmlError)));
    }

    if (doc.Error())
    {
        LOGE_TAG("HarmonyXMLParser", "doc error: %s: %s", doc.ErrorName(), doc.ErrorStr());
        AddError(doc.ErrorName(), doc.ErrorStr());
        LOGD_TAG("HarmonyXMLParser", "---------------------------Document:\n\n%s\n\n", data.c_str());
...
Words HarmonyXMLParser::ParseWords(XMLElement* element)
{
    Words words = Words();
...
std::shared_ptr<MetronomeMark> HarmonyXMLParser::ParseMetronomeMark(XMLElement* element)
{
...
Dynamic HarmonyXMLParser::ParseDynamicElement(XMLElement* element)
{
...
std::shared_ptr<DynamicWedge> HarmonyXMLParser::ParseDynamicWedgeElement(XMLElement* element, float currentTimeInMeasure, int measureIndex)
{
    if (element)
    {
...
std::shared_ptr<BracketDirection> HarmonyXMLParser::ParseDashesDirectionElement(XMLElement* element, float currentTimeInMeasure, int measureIndex)
{
    if (element)
    {
...
std::shared_ptr<BracketDirection> HarmonyXMLParser::ParseBracketDirectionElement(XMLElement* element, float currentTimeInMeasure, int measureIndex)
{
    if (element)
    {
...
std::shared_ptr<DurationDirection> HarmonyXMLParser::ParseDurationDirection(XMLElement* directionElement, bool& isNewDirection, float currentTimeInMeasure, int measureIndex)
{
...
void HarmonyXMLParser::ParseWorkElement(XMLElement* workElement, std::string& workTitle, int& workNumber)
{
    if (workElement)
    {
        // work title
...
void HarmonyXMLParser::ParseEncodingElement(XMLElement* encodingElement, const std::shared_ptr<Song>& song)
{
    if (encodingElement)
    {
        // loop through all elements
...
void HarmonyXMLParser::ParseIdentificationElement(XMLElement* idElement, const std::shared_ptr<Song>& song)
{
    if (idElement)
    {
        // loop through all elements
...
MusicDisplayConstants HarmonyXMLParser::ParseDefaultsElement(XMLElement* defaultsElement)
{
...
void HarmonyXMLParser::ParseFrameElement(XMLElement* frameElement, Chord& chord)
{
    if (frameElement)
    {
...
void HarmonyXMLParser::ParseHarmonyElement(XMLElement* harmonyElement, float& currentTimeInMeasure, std::vector<std::shared_ptr<Measure>> currentMeasures)
{
    Chord newChord;
...
Barline HarmonyXMLParser::ParseBarlineElement(XMLElement* barlineElement)
{
...
void HarmonyXMLParser::ParseArpeggioElement(XMLElement* element, const std::shared_ptr<Measure>& currentMeasure, const std::shared_ptr<Note>& currentNote)
{
    if (element)
    {
...
std::shared_ptr<Marker> HarmonyXMLParser::ParseCodaSegnoElement(XMLElement* element)
{
    if (!element)
...
void HarmonyXMLParser::ParseSlurElement(XMLElement* element, const std::shared_ptr<Measure>& currentMeasure, const std::shared_ptr<Note>& currentNote)
{
    if (element)
    {
...
void HarmonyXMLParser::ParseEndingElement(XMLElement* element, const std::shared_ptr<Song>& song, std::vector<std::shared_ptr<Measure>> currentMeasures)
{
    if (!element)
        throw IsNullException();
...
void HarmonyXMLParser::ParsePart(const std::shared_ptr<Song>& song, XMLElement* part, bool isFirstPart, MusicDisplayConstants displayConstants)
{
